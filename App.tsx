...
const FlameIcon = ({ className = '' }: { className?: string }) => (
  <svg viewBox="0 0 24 24" className={className} aria-hidden="true" focusable="false">
    <path
      d="M12.2 2.2c.2 2.4-.6 4.2-2.1 5.9C8.8 9.7 8 11.2 8 13c0 2.6 1.8 4.6 4 4.6s4-2 4-4.6c0-1.4-.5-2.6-1.5-3.8-.7-.8-1.3-1.8-1.6-3.1-.8 1-1.7 1.8-2.7 2.4.2-1.9.1-3.8.0-6.3Z"
      fill="currentColor"
    />
    <path
      d="M12 9.6c-.6.7-1.4 1.4-2 2.1-.7.9-1 1.7-1 2.7 0 1.7 1.2 3 3 3s3-1.3 3-3c0-.9-.3-1.7-1-2.6-.4-.6-.7-1.2-1-1.9-.4.5-.9 1-1.5 1.4.2-.7.3-1.2.5-1.7Z"
      fill="#FBBF24"
      fillOpacity="0.95"
    />
  </svg>
...
const LockIcon = ({ className = '' }: { className?: string }) => (
  <svg viewBox="0 0 24 24" className={className} aria-hidden="true" focusable="false">
    <path d="M8 10V8a4 4 0 1 1 8 0v2" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    <rect x="6.5" y="10" width="11" height="10" rx="2" fill="none" stroke="currentColor" strokeWidth="2" />
    <path d="M12 14v3" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
  </svg>
);
...
function App() {
  ...
  const [teacherPermissionLevel, setTeacherPermissionLevel] = useState<'limited' | 'full'>('limited');
  ...
  const handleDeleteWord = async (word: Word) => {
    if (!confirm(`האם אתה בטוח שברצונך למחוק את המילה "${word.aramaic}" (${word.hebrew})?`)) {
      return;
    }
  ...
  const startGame = (sugia?: Sugia, skipIntro: boolean = false) => {
    ...
    setStats({
        ...
        sugiaTitle: sugia?.title || (customWordList ? 'תרגול מורה' : 'פתיחת היחידה')
    });
...
const equipSkin = (id: string) => {
  ...
  const ControlsDisplay = () => (
    <div className="mt-4 md:mt-8 rk-glass p-4 rounded-xl border border-slate-700/60 text-sm text-slate-300 shadow-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-right">
        ...
        <div className="flex justify-between gap-4"><span>ירי:</span> <span className="text-amber-400 font-bold">{isMobile ? "כפתור אש" : "קליק / רווח"}</span></div>
  ...
  return (
    ...
    >
      ...
      {unlockNotification && (
        <div
          className="absolute left-1/2 -translate-x-1/2 z-[200] pointer-events-none w-full px-4 max-w-xs sm:max-w-sm bottom-[calc(7rem+env(safe-area-inset-bottom))] md:bottom-10"
          data-ui="true"
        >
          {/* Important: animate an inner wrapper so we don't override Tailwind's centering transform */}
          <div className="animate-bounce-slow">
            <div className="bg-gradient-to-r from-amber-600 to-yellow-400 p-1 rounded-2xl shadow-2xl">
              <div className="bg-slate-900 rounded-xl px-3 py-2 md:px-8 md:py-4 flex items-center gap-3 md:gap-6 border border-amber-400/30">
                <span className="text-2xl md:text-5xl leading-none">{unlockNotification.icon}</span>
                <div className="text-right">
                  <div className="text-amber-400 font-black text-[10px] md:text-sm uppercase tracking-widest">הישג חדש!</div>
                  <div className="text-white font-black text-base md:text-2xl leading-tight">{unlockNotification.title}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {gameState === 'PLAYING' && (
        <div className="absolute inset-0 pointer-events-none rk-safe-overlay flex flex-col justify-between">
            <div className="flex justify-between items-start gap-2" data-ui="true">
                <div className="rk-hud-panel rounded-xl md:rounded-3xl p-1.5 md:p-4 min-w-[112px] md:min-w-[220px]">
                    <div className="flex items-start justify-between gap-3">
                      <div className="text-amber-400 font-black text-base md:text-2xl flex items-center gap-1 md:gap-2 leading-none">
                        <GoldCoin size={isMobile ? 14 : 18} /> {displayScore.toLocaleString()}
                      </div>
                      <div className="text-right">
                    ...
                    <div className="mt-1.5 text-slate-200 text-[9px] md:text-xs font-bold uppercase tracking-widest truncate">{stats.sugiaTitle}</div>
                ...
                <div className="text-center absolute left-1/2 -translate-x-1/2 top-4 md:top-6 w-full max-w-[200px] md:max-w-lg z-10">
                    <div
                         dir="auto"
                         className="font-aramaic text-3xl md:text-6xl text-white drop-shadow-[0_0_15px_rgba(251,191,36,0.6)] md:drop-shadow-[0_0_25px_rgba(251,191,36,0.6)]"
                ...
                <div className="flex flex-col items-end gap-2">
                    <button
                      ...
                      className="rk-hud-panel pointer-events-auto w-11 h-11 md:w-14 md:h-14 rounded-full flex items-center justify-center border border-blue-400/25 active:scale-90"
                      aria-label="עצור / המשך"
                    >
                      <PauseIcon className="w-5 h-5 md:w-6 md:h-6 text-slate-100" />
                    </button>
                ...
                <div className="flex flex-col gap-4 items-center pointer-events-auto" data-ui="true">
                    {isMobile && (
                      <button 
                        ...
                        aria-label="אש"
                      >
                        <FlameIcon className="w-8 h-8 text-orange-300 drop-shadow-[0_0_14px_rgba(249,115,22,0.65)]" />
                      </button>
                    )}
                </div>
            </div>
                  ...
                  <div className="space-y-6 md:space-y-8">
                      ...
                      <section className="rk-glass p-6 rounded-3xl border border-slate-800/60">
                          ...
                          <p className="text-sm md:text-lg text-slate-300 leading-relaxed">
                              במרכז המסך מופיעה מילה באנגלית (למשל: "apple"). <br/>
                      ...
                      <section className="rk-glass p-6 rounded-3xl border border-slate-800/60">
                          ...
                          <div className="text-sm md:text-lg text-slate-300 space-y-3">
                              ...
                              <p><span className="text-amber-400 font-bold">בונוס רצף (Combo):</span> כל פגיעה רצופה מכפילה את הניקוד! רצף של 10 פגיעות ומעלה מזכה בתואר "מאסטר קומבו".</p>
                      ...
                      {/* Section 4: Stages */}
                      <section className="rk-glass p-6 rounded-3xl border border-slate-800/60">
                          <h3 className="text-xl md:text-3xl font-aramaic text-purple-400 mb-4 font-bold border-b border-purple-900/30 pb-2">🌍 סוגי השלבים (יחידות)</h3>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs md:text-sm text-slate-300">
                              <div className="rk-glass p-2 rounded border border-slate-800/60">🌊 <span className="font-bold">חימום:</span> אויבים נעים בגל</div>
                              <div className="rk-glass p-2 rounded border border-slate-800/60">🔥 <span className="font-bold">מטר אותיות:</span> אותיות נופלות</div>
                              <div className="rk-glass p-2 rounded border border-slate-800/60">📜 <span className="font-bold">מיקרו-מילים:</span> מילים קטנות ומהירות</div>
                              <div className="rk-glass p-2 rounded border border-slate-800/60">💨 <span className="font-bold">רוח צד:</span> רוח סוחפת את המטוס</div>
                              <div className="rk-glass p-2 rounded border border-slate-800/60">👁️ <span className="font-bold">הבהוב:</span> אויבים נעלמים ומופיעים</div>
                              <div className="rk-glass p-2 rounded border border-slate-800/60">🌑 <span className="font-bold">חושך:</span> רואים רק קרוב</div>
                          </div>
                      </section>
                  </div>
          ...
          <div className="absolute inset-0 bg-transparent flex flex-col z-[100] p-4 md:p-8 overflow-y-auto md:overflow-hidden h-full">
              <div className="max-w-4xl w-full mx-auto flex flex-col h-full text-white">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-auto md:h-full overflow-visible md:overflow-hidden pb-10 md:pb-0">
                    ...
                    <div className="flex flex-col h-[55vh] md:h-full overflow-hidden rk-glass p-4 rounded-3xl border border-slate-800/60">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 mb-4 scrollbar-hide">
                            {filteredTeacherDictionary.length === 0 ? (
                                ...
                                const isDynamicWord = dynamicWords.some(dw => dw.aramaic === word.aramaic && dw.hebrew === word.hebrew && dw.cat === word.cat);
                                return (
                                    <div key={word.originalIndex} 
                                      className={`p-3 rounded-xl border-2 flex justify-between items-center transition-all group
                                        ...
                                        <div className="text-right flex-1" onClick={() => toggleTeacherWordSelection(word.originalIndex)} style={{ cursor: 'pointer' }}>
                                            <div className="font-aramaic text-lg text-white" dir="auto">{word.aramaic}</div>
                                            <div className="text-[10px] text-slate-500 font-bold">{word.hebrew}</div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {teacherPermissionLevel === 'full' && isDynamicWord && (
                                                <button 
                                            ...
                                            <div onClick={() => toggleTeacherWordSelection(word.originalIndex)} style={{ cursor: 'pointer' }}
                                                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-blue-500 border-blue-500' : 'border-slate-700'}`}>
                    ...
                    {/* Right Side: Add New Word (only for full permissions) */}
                    {teacherPermissionLevel === 'full' && (
                        <div className="rk-glass-strong p-6 rounded-3xl border border-blue-500/20 flex flex-col h-auto md:h-auto">
                            ...
                            <div className="space-y-4 flex-1">
                                <div>
                                    <label className="text-slate-400 text-xs block mb-1">מילה באנגלית</label>
                                    <input
                                        type="text"
                                        dir="ltr"
                                        value={newWordAramaic}
                                        onChange={e => setNewWordAramaic(e.target.value)}
                                        placeholder="לדוגמא: apple"
                                        className="w-full rk-glass border border-slate-700/60 rounded-xl p-4 text-left text-white outline-none focus:border-blue-500"
                                    />
                                </div>
                                <div>
                    ...
                    {teacherPermissionLevel === 'limited' && (
                        <div className="rk-glass-strong p-6 rounded-3xl border border-blue-500/20 flex flex-col h-auto md:h-auto items-center justify-center">
                            <h3 className="rk-neon-subtitle font-black text-xl mb-4 text-center">מצב מורה מוגבל</h3>
                            <p className="text-slate-400 text-center text-sm">
                                במצב זה ניתן לבחור מילים וליצור קישור לשיעור בלבד.
                                <br />
                                <br />
                                לגישה מלאה (הוספה ומחיקת מילים) נדרש קוד נוסף.
                            </p>
                        </div>
                    )}
                  </div>
      ...
      {gameState === 'MAP' && (
          <div className="absolute inset-0 bg-transparent flex flex-col z-[50] overflow-hidden h-full text-white">
              <div className="relative z-10 p-3 md:p-6 flex justify-between items-center rk-glass-strong border-b border-blue-500/10">
                <button onClick={handleReturnToMenu} className="rk-btn rk-btn-muted px-4 py-2 md:px-8 md:py-3 text-xs md:text-lg">חזור</button>
                <h2 className="text-2xl md:text-6xl font-aramaic rk-neon-title font-black tracking-tighter">מפת היחידות</h2>
                                  ...
                                  <div onClick={() => { if(isUnlocked) { Sound.play('ui_click'); setSelectedSugia(sugia); } }}
                                      ...
                                      <div className={`flex items-center justify-center flex-1 ${isUnlocked ? 'rk-neon-title' : 'text-slate-400'}`}>
                                        {isUnlocked ? (
                                          ...
                                          <LockIcon className="w-7 h-7 md:w-12 md:h-12" />
                                        )}
                                      </div>
      ...
      {gameState === 'ACHIEVEMENTS' && (
          <div className="absolute inset-0 bg-transparent flex flex-col items-center p-4 md:p-8 z-20 overflow-y-auto scrollbar-hide h-full text-white">
              <div className="w-full max-w-4xl">
                  <h2 className="text-3xl md:text-7xl font-aramaic text-purple-300 text-center mb-6 md:mb-12 drop-shadow-[0_0_24px_rgba(88,28,135,0.35)]">הישגים</h2>
...
const Backdrop = ({ mode, showShips }: { mode: BackdropMode; showShips: boolean }) => {
  const glyphs = useMemo(() => {
    const chars = [
      ...
      'hello','please','thanks','yes','no','learn','read','write','school','game','level'
    ];
    const palette = [
